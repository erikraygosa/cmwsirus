...
@section('title', 'Expediente — ' . $operador->Operador)

@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:.5rem;">
        <div>
            <a href="{{ route('expedientes-operadores.index') }}" class="btn btn-sm btn-outline-secondary mr-2">
            ...
            <span class="badge badge-secondary ml-2">ID {{ $operador->IdOper }}</span>
            @if($completo)
...
@section('content')
    ...
    <div class="row">
        @foreach ($catalogo as $clave => $doc)
            @php
                ...
                $fechaVig   = $campoVig && $operador->{$campoVig} ? \Carbon\Carbon::parse($operador->{$campoVig}) : null;
                    ...
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if($campoVig)
                            <div class="mb-2">
                                ...
                                <form action="{{ route('expedientes-operadores.vigencia.update', $operador->IdOper) }}"
                        ...
                        <div class="mt-auto">
                            @if($subido)
                                <div class="btn-group w-100" role="group">
                                    ...
                                    <form action="{{ route('expedientes-operadores.destroy', ['idAdj' => $adjId]) }}"
                                          method="POST"
                                          style="flex:1;">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="modal fade" id="modalSubir" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formSubir"
                      action="{{ route('expedientes-operadores.store', $operador->IdOper) }}"
                      method="POST"
    ...
    <div class="modal fade" id="modalZip" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('expedientes-operadores.zip', $operador->IdOper) }}" method="GET">
                    <div class="modal-header">
...
(function () {
    ...
    document.getElementById('btnGuardarCam').addEventListener('click', function () {
        ...
        const ts    = Date.now();
        const action = '{{ route('expedientes-operadores.store', $operador->IdOper) }}';
